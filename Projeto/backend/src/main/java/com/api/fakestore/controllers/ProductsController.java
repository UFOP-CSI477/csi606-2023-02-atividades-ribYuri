...
public class ProductsController {
    ...
    @GetMapping()
    public ResponseEntity<Object> indexProducts(@RequestParam(defaultValue = "id") String column,
                                                @RequestParam(defaultValue = "asc") String order ) {
        return ResponseEntity.ok().body(productsServices.findAll(column, order));
    }

    @GetMapping("/search")
    public ResponseEntity<Object> searchProducts(@RequestParam String query) {
        return ResponseEntity.ok().body(productsServices.searchProducts(query));
    }
